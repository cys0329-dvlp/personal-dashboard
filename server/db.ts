...
// ---- Admin Accounts ----
import { adminAccounts, InsertAdminAccount, userDataStorage } from "../drizzle/schema";

export async function createAdminAccount(account: InsertAdminAccount) {
  const db = await getDb();
  if (!db) {
    throw new Error("Database not available");
  }

  return await db.insert(adminAccounts).values(account);
}
...
export async function checkAdminAccountExists() {
  ...
  const result = await db.select().from(adminAccounts).limit(1);
  return result.length > 0;
}

// ---- User Data Storage ----
export async function saveUserData(userId: number, dataType: string, data: any) {
  const db = await getDb();
  if (!db) {
    throw new Error("Database not available");
  }
  ...
  const existing = await db
    ...
    .where(eq(userDataStorage.userId, userId))
    .limit(1);
  ...
  if (existing.length > 0) {
    ...
    return await db.insert(userDataStorage).values({
      userId,
      dataType,
